...
def store(text: str, categories: list[MemoryCategory]) -> str:
    """存储记忆。LLM 自动从文本中提取事实并存储。Args: text=记忆文本（可包含多个事实）；categories=必填，USER=用户历史/偏好/背景，FACT=规则/事实/知识，EXP=技能/项目经历"""
    if not categories:
...
def search(query: str, category: MemoryCategory) -> dict:
    """搜索记忆，返回文本和相关性分数。Args: query=搜索关键词；category=必填，USER=用户历史/偏好/背景，FACT=规则/事实/知识，EXP=技能/项目经历"""
